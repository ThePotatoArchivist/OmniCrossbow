...
public class EmberEntityRenderer extends EntityRenderer<EmberEntity> {
    Identifier TEXTURE = Identifier.ofVanilla("textures/particle/lava.png");
    ...
    public void render(EmberEntity entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        vertex(buffer, positionMatrix, entry, LightmapTextureManager.MAX_LIGHT_COORDINATE, 0, 0, 0, 1);
        vertex(buffer, positionMatrix, entry, LightmapTextureManager.MAX_LIGHT_COORDINATE, 1, 0, 1, 1);
        vertex(buffer, positionMatrix, entry, LightmapTextureManager.MAX_LIGHT_COORDINATE, 1, 1, 1, 0);
        vertex(buffer, positionMatrix, entry, LightmapTextureManager.MAX_LIGHT_COORDINATE, 0, 1, 0, 0);
        matrices.pop();
        super.render(entity, yaw, tickDelta, matrices, vertexConsumers, light);
    }

    private static void vertex(VertexConsumer buffer, Matrix4f matrix, MatrixStack.Entry entry, int light, float x, int y, int u, int v) {
